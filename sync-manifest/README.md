...
steps:
  - id: deploy-platform
    ...
    with:
      ...
      token: $TOKEN
  - id: sync-manifest
    name: Sync manifest with project repositories
    uses: Telefonica/cross-platform-actions/sync-manifest@{BRANCH_NAME|VERSION}
    with:
...
- `manifest` - Manifest to sync as a Github Secret in the target repositories.
- `project` - Project information as a JSON string.
- `secret-name` - Name of the Github Secret to sync the manifest to.
