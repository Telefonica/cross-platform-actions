...
- The deploy workflow creates a job with a step named `Set ID ({{inputs.id}})`. This step will be used to verify that the workflow is the one that was launched by this action. The `id` input is a random uuid generated by this action on each execution.
